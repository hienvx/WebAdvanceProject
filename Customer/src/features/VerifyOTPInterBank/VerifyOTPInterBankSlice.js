...
export const doGetOTPInfoThunk = createAsyncThunk(
  ...
  async (data, thunkAPI) => {
    ...
    if (response.status) {
      ...
      thunkAPI.dispatch(
        updateValue({
          value: moment.unix(response.data.time).format("DD/MM/YYYY hh:mm:ss"),
          option: ["time"],
        })
      );
    } else {
...
export const doConfirm = createAsyncThunk(
  ...
  async (data, thunkAPI) => {
    ...
    const response = await confirmOTP(
      thunkAPI.getState().verifyOTPInterBankSlice.otp
    );
    if (response.status) {
      // Go to message page
...
let confirmOTP = async function (state) {
  return await axios
    .post("http://localhost:3000/transfer/interbank/confirm-tranfer", {
...
export const verifyOTPInterBankSlice = createSlice({
  name: "verifyOTPInterBankSlice",
  initialState: {
    userNumberAccount: "",
    targetNumberAccount: "",
    targetName: "",
...
export const otpModel = (state) => state.verifyOTPInterBankSlice;
export const { updateValue, resetValue } = verifyOTPInterBankSlice.actions;
export default verifyOTPInterBankSlice.reducer;
