...
export const doGetUserAccountThunk = createAsyncThunk(
    ...
    async (data, thunkAPI) => {
        ...
        if(response.status){
            thunkAPI.dispatch(updateValue({value: response.data.profile.fullName, option: ["message"]}));
        }else{
...
let getUserAccount = async function (state) {
    return await axios.get("http://localhost:3000/accounts/UserAccount/" + state.userAccount)
        .then(response => {
...
let getNumberAccount = async function (state) {
    return await axios.get("http://localhost:3000/accounts/NumberAccount/" + state.numberAccount)
        .then(response => {
            return response.data
        })
        .catch(() => {
...
export const doPaymentUserAccountThunk = createAsyncThunk(
    ...
    async (data, thunkAPI) => {
        ...
        if (state.userAccount == "") {
            thunkAPI.dispatch(updateValue({value: "User account must be not empty", option: ["message"]}));
            return;
        }
        thunkAPI.dispatch(updateValue({value: true, option: ["isLoading"]}));
        thunkAPI.dispatch(updateValue({value: "", option: ["message"]}));
...
export const doPaymentNumberAccountThunk = createAsyncThunk(
    ...
    async (data, thunkAPI) => {
        ...
        if (state.numberAccount == "") {
            thunkAPI.dispatch(updateValue({value: "Number account must be not empty", option: ["message"]}));
            return;
        }
...
let paymentUserAccount = async function (state) {
    return await axios.post("http://localhost:3000/accounts/payment/Account",
        {
            data: {
...
let paymentNumberAccount = async function (state) {
    return await axios.post("http://localhost:3000/accounts/payment/NumberAccount",
        {
            data: {
